	...
	*/
	...
	Tree.prototype.clickHandler = function (event) {
		...
		if ((classList.indexOf('expand-icon') !== -1)) {
            ...
            if(event.which == 0 || event.which == 1){ // 左键点击
                ...
                this.render();
            }else if(event.which == 3||event.which == 4||event.which == 5||event.which == 6){ // 右键点击
				if(node.level === 3||node.level === 4||node.level === 5||node.level === 6){ // 在每一个图层的根节点上右击才会弹出图层列表，子节点和父节点上右击都不会
	...
	Tree.prototype.buildTree = function (nodes, level) {
		...
		$.each(nodes, function addNodes(id, node) {
			...
			if(level === 3||level === 4){ // 图层级别的列表项才对应图层属性对话框
                treeItem.attr('title', '右键定位');
			}
